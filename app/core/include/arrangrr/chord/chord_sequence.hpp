...
class ChordSequence {
 public:
  ...
  constexpr std::size_t count() const noexcept { return steps_.size(); }
  constexpr const ChordStep& step(std::size_t i) const noexcept { return steps_[i]; }
  constexpr Span<const ChordStep> steps() const noexcept { return steps_.span(); }
  ...
  constexpr Tick length() const noexcept {
    ...
    const ChordStep& last = steps_[steps_.size() - 1];
  ...
  [[nodiscard]] constexpr bool append(std::int8_t degree, std::int8_t quality_ovr,
    ...
    return steps_.push_back(
        ChordStep{length(), duration, degree, quality_ovr, velocity});
  }

  // Removes step i and closes the gap (later steps shift earlier).
  constexpr bool remove(std::size_t i) noexcept {
    if (i >= steps_.size()) return false;
    const Tick removed = steps_[i].duration;
    steps_.erase(i);
    for (std::size_t k = i; k < steps_.size(); ++k) steps_[k].start -= removed;
    return true;
  }

  constexpr void clear() noexcept { steps_.clear(); }
  ...
  constexpr ChordStep* last() noexcept {
    return steps_.empty() ? nullptr : &steps_[steps_.size() - 1];
  }
  ...
  constexpr void quantize(Tick grid = kTicksPerBar) noexcept {
    if (grid == 0 || steps_.empty()) return;
    Tick previous_end = 0;
    for (std::size_t i = 0; i < steps_.size(); ++i) {
      Tick snapped = ((steps_[i].start + grid / 2) / grid) * grid;
      if (snapped < previous_end) snapped = previous_end;  // keep order
      steps_[i].start = snapped;
      previous_end = snapped + grid;  // provisional; fixed below
    }
    for (std::size_t i = 0; i + 1 < steps_.size(); ++i) {
      const Tick gap = steps_[i + 1].start - steps_[i].start;
      steps_[i].duration = gap > 0 ? gap : grid;
    }
    ChordStep& tail = steps_[steps_.size() - 1];
    tail.duration = ((tail.duration + grid / 2) / grid) * grid;
    if (tail.duration == 0) tail.duration = grid;
  }
  ...
  constexpr void transpose_to(std::uint8_t root_pc, std::int8_t mode) noexcept {
    ...
    if (mode >= 0 && mode < kModeCount) key.mode = static_cast<Mode>(mode);
  }
 ...
 private:
  StaticVector<ChordStep, kMaxChordSteps> steps_;
};
