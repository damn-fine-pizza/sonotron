...
6. **Scale-degree melodic parts + synchronized break** — *machinery landed (D39/D40/D41/D42):* the `Key` is threaded into `resolve()`, a per-event `NoteSource` (`kScaleDegree`/`kInterval`) walks scale degrees or chord-root intervals, the resolution path is now a pipeline with `VoicingPolicy::kLead` voice-leading and a `ChordGesture` strum/roll stage. **Remaining:** actually author `kLead`/`kPhrase`/melodic + gesture + voice-led content in the styles (see `docs/research/yamaha-style-corpus-and-rules.md` for the 79 genre rules), and author real `kBreak` content.
