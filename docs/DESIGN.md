...
6. **Scale-degree melodic parts + synchronized break** — thread the `Key` (not just `ChordState`) into `resolve()` so a `kLead`/`kPhrase` role can walk scale degrees instead of only chord tones; author real `kBreak` content once the melodic machinery exists to make a break musically interesting rather than just silence.
