...
| — | *Status:* understanding ~99%; **all of Francesco's red blockers resolved** (D26 #1/#2, D27 #3, D28 C2, D29 C3); maintenance folded in; **budgets anchored (D33)**. Core M0–M5 spine, the live piano→chord harmonizer (D34), program/voice selection (D35) and the all-roles register/voice enablers (D36) are landed; Host UI **H1+H2 landed** (§27); 8-style-parts, the groove engine and the live-keyboard `ArpeggiatorEngine` have since landed too. The **pattern-relative note vocabulary** (D39: `NoteSource` chord-tone/scale-degree/interval) and the **host UDS-JSONL control adapter** (D38 enabling step) have now landed as well. **Next (§27 gap analysis + `docs/reflections/hybrid-arranger-gap-analysis.md` + `docs/reflections/live-daw-around-arrangrr-ux.md`):** `ChordGesture` (the generative layer atop D39), voice-leading + harmonic spillover, the reversible MIDI looper, then the Generative Director (D37) capstone; the Dear ImGui client (D38) rides on the now-landed UDS-JSONL adapter, off the core's critical path. | 🔄 In progress |
...
|---|---|---|---|
| **Arranger** | ~60% | Chord recognition (triads, inversions via shell/lowest-root, 7ths, incomplete chords, single-finger, chord-memory latch); harmonic state + live change; 13 `SectionType`s; NTT relative patterns (root/fifth/octave/chord-tone, auto major/minor third, seventh); relative→MIDI `resolve()`; part coordination (3→8 roles in progress, D36); one-shot fills that return to the variation; crash accents. | Voicing is **close-position only** — no open voicing, no inversion policy, no voice-leading, no max-jump clamp, no extensions on/off toggle. ~~No scale-degree melodic resolution~~ **(resolved, D39):** `resolve()` now takes the live `Key` (threaded from `ChordEngine::key()`) and a per-event `NoteSource` lets a role read scale degrees (`kScaleDegree`) or intervals/tensions from the chord root (`kInterval`), not only chord tones — melodic lead/phrase lines are now expressible. Still open: no style yet *authors* such parts, and no `ChordGesture` generative layer. `SectionType::kBreak` is defined (§11 table) but referenced by **zero** style files — no synchronized break exists in practice. No explicit slash-chord/bass-note (on-bass) resolver, despite §11 describing the policy. |
...
6. **Scale-degree melodic parts + synchronized break** — *machinery landed (D39):* the `Key` is now threaded into `resolve()` and a per-event `NoteSource` (`kScaleDegree`/`kInterval`) lets `kLead`/`kPhrase` walk scale degrees or chord-root intervals. **Remaining:** actually author `kLead`/`kPhrase`/melodic content in the styles using it, add the `ChordGesture` generative layer on top, and author real `kBreak` content now that the melodic machinery exists.
