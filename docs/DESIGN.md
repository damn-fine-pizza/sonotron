...
| D33 | **Anchor STM32 = STM32H743 (Cortex-M7 @480 MHz, 1 MB RAM, 2 MB flash) + envelope disciplinato 512 KB** (fix Francesco M2/M3). Non è la scelta hardware finale (resta M13): è il **chiodo** che rende i budget falsificabili. Regole: **(a)** tutti i pool `MAX_*` devono stare in **≤512 KB** (metà del part) con `static_assert(TOTAL_POOLS ≤ 512*1024)`; **(b)** hot path (code I/O, scheduler, note tracker) nei **192 KB DTCM** zero-wait (deterministic-by-design, evita di ragionare sulle cache D/I del M7); pool grossi in AXI SRAM; **(c)** **contenuti compilati read-only (style/pattern di fabbrica) in FLASH, memory-mapped `const`, letti in-place** (il formato §21 è indici+POD) — la RAM è solo per il *mutabile* (loop, registrazioni, stato live); **(d)** `Event` = **8 byte POD** (`{u32 tick; u8 status; u8 d1; u8 d2; u8 meta}`, NoteOff = evento separato, `static_assert(sizeof(Event)==8)`); **(e)** budget derivati, non a occhio: looper 8×3072 ev (192 KB) · live-rec 8k ev (64 KB) · user content 8k ev (64 KB) · chordseq 16×128 step (24 KB) · scheduler 4096×12 B (48 KB) · ring I/O 4 porte (16 KB) · tracker/routing/state (24 KB) · headroom ~80 KB ⇒ **≤512 KB**. Sostituisce "~64k eventi/project": ora **≤40k eventi RAM-resident** + read-only illimitati-in-pratica in flash. | ✅ Fermo |
| — | *Stato:* comprensione ~99%; **tutti i blocker rossi di Francesco risolti** (D26 #1/#2, D27 #3, D28 C2, D29 C3); manutenzione foldata; **budget ancorati (D33)**. Prossimo: M0. | 🔄 In corso |
...
- **Budget espliciti, ancorati (D33)**: anchor **STM32H743** (1 MB RAM / 2 MB flash), envelope pool totali **≤512 KB** verificato da `static_assert`; hot path nei **192 KB DTCM** zero-wait; contenuti compilati read-only **in flash memory-mapped** (RAM solo per il mutabile); `sizeof(Event)==8` asserito; tick jitter < 1 ms (per inizio-messaggio), latenza in→out < 3 ms. Un **validator** (tool) rifiuta project che sforano i limiti prima dell'export.
...
| **Max events (looper/pattern)** | 3072 ev/loop-track (×8 track), ≤40k eventi RAM-resident totali; read-only in flash (D33) |
| **Max sections/style** | ~16 (Intro×2, Var×4, Fill×4, Break, End×2 + margine) |
| **Max projects / storage** | anchor D33: 2 MB flash → ~512 KB contenuti factory + slot project; SD opzionale per di più |
