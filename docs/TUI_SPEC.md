...
This document consolidates the roadmap for the "piano keyboard + MIDI monitor" panel of the host TUI (`components/hostrt/console.*`, `components/hostrt/shell.*`, and new files under `components/hostrt/`). It follows DESIGN.md's conventions: D-numbered decisions are locked facts from the parent design; this document adds its own numbered sections but does not mint new D-numbers — where it depends on a DESIGN.md decision, it says so explicitly (e.g. "per D26").
