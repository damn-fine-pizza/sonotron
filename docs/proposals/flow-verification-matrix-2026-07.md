...
| **Real-UI automation (DEFERRED)** | Verifying an actual mouse click / actual pixels, not a state mutation that mimics one | explicitly out of scope per owner steer — every headless test below that "arms" a button (`click_arm_auto_song`) mutates the exact fields the click would mutate because **there is no click-injection seam for `ImGui::SmallButton` in this codebase** (both `test_grid_panel_auto_song.cpp:87` and `test_grid_panel_auto_song_real_backend.cpp:81` say this explicitly) |
...
|---|---|---|---|---|
| Press Play -> transport starts | `transport_panel.cpp:55` `send("transport start")` + `app_state.note_transport_sent(true)` | `test_in_process_brain_session_bar_advance.cpp` (functional) sends the SAME literal string through a real `InProcessBrainSession`; render-side button click itself is NOT automated (no click-injection seam) | End-to-end on the BACKEND half (real engine, real ring, real bar climb); the BUTTON→send() call site itself is exercised only by grep-level trust that transport_panel.cpp calls send() with this exact string | Adequate for now. Residual gap: nothing pins that `render_transport_panel`'s Play button, when clicked, calls `send()` with exactly this string — only a `render_one_frame`-style headless harness (as `test_grid_panel_auto_song_real_backend.cpp` already established for the grid) would close it. **Hand to Torquato** if judged worth the cost. |
| Press Stop -> transport stops, bar parks to 0 | `transport_panel.cpp:61` `send("transport stop")`; reduction in `src/app_state.cpp:80-94` (`kTransport` "stopped" case zeroes `m_bar/m_beat/m_pulse`) | `test_app_state.cpp` (unit, pins the reduction only) | STUBBED at the reduction layer (unit-tests `AppState::apply_line` directly, no real backend, no button) | No functional coverage of "Stop" through a real backend exists today (unlike Play, which `test_in_process_brain_session_bar_advance.cpp` now covers). **Gap: hand to Torquato** — a real-backend `transport stop` round trip (send stop, poll, assert `bar()==0` and `transport()==kStopped`), mirroring the Play test's shape. |
...
| Click a style leaf while playing -> `style switch <name>` (live morph, quantized) | `browser_panel.cpp:110-115` | `NONE` | NONE | **Gap.** This is a DIFFERENT code path from `style load` (mid-song morph vs hard reload) and has zero coverage, functional or unit. **Hand to Torquato.** |
...
| Click a FILLED cell -> real `launch clip <id> quantize <n>` + opens Sequence Edit | `grid_panel.cpp:526-546` | `NONE` end-to-end; `test_grid_model.cpp` covers `next_scene_to_launch`/model shape only, not this click handler | NONE for this exact trigger | **Gap: hand to Torquato** for a headless `render_grid_panel`-driving test that simulates the click's STATE EFFECT (the pattern `test_grid_panel_auto_song*.cpp` already established for the header button) and asserts the `launch clip` wire text, real backend optional depending on cost/benefit judgment. |
...
| Click scene-header ▶ -> `style section <name>` + `launch scene <s> quantize <n>`, sets `fx.active_scene` baseline | `grid_panel.cpp:397-421` | `NONE` directly; `test_grid_model.cpp` covers the PURE decision function only | NONE for this exact trigger | **Gap: hand to Torquato**, same class as the filled-cell click above. |
...
| M/S latch click -> real `part <token> mute\|solo on\|off` + `PartsModel` toggle | `grid_panel.cpp:460-469` | `NONE` for the click; `test_parts_model.cpp` (unit) covers `PartsModel::toggle_mute/toggle_solo` in isolation | Partial | **Gap: hand to Torquato** for a headless-render click-equivalent test asserting the exact wire string, mirroring the established pattern. |
...
| Auto-song bookkeeping surviving a transport stop/restart (bar rewinds to 0) | `grid_panel.cpp:283-292` | `test_grid_panel_auto_song.cpp`'s `test_auto_song_stuck_after_transport_stop_then_restart` — SAME `SpyBrainSession`/injected-beat shape as above | STUBBED (same caveat as the row above) | No real-backend equivalent exists yet for the stop/restart case specifically (the real-backend test only exercises the "never stopped" happy path). **Gap: hand to Torquato** — extend `test_grid_panel_auto_song_real_backend.cpp`'s shape (or a sibling file) to send `"transport stop"` then a fresh `"transport start"` through the real backend and assert the anchor re-seats correctly. |
...
- **Owner fork, flagged not decided:** is a CI-wired headless
  ...
  GL/EGL context)? This needs a feasibility check from Corelli/Nazzareno
