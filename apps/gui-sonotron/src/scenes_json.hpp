...
// GridModel::scene_section's own storage shape). Fewer than kMaxSceneCount
// entries in EITHER array leaves the remaining indices at whatever `model`
// already had (its own constructor default: the bare column number for
// names, GridModel::kDefaultSectionType for sections); more than
// kMaxSceneCount entries are ignored past that bound, forward-compatible
// with a file written by a future binary with a larger grid. The "sections"
// key is OPTIONAL: a scenes.json written by a pre-SLICE-4a binary (no
// "sections" key at all) parses cleanly and every scene keeps the
// constructor default section -- old files are not a parse failure. Returns
// true on success; on failure returns false and fills `error` with a
// 1-based line/column message, leaving `model` partially updated (same
// "parse failure is a whole-file failure" discipline as layout_json.hpp's
...
// Serializes every one of `model`'s kMaxSceneCount scene names AND sections
// to canonical JSON text (deterministic key order -- "scenes" then
// "sections" -- fixed 2-space indentation, no trailing newline -- callers
// add one), the exact on-disk format read back by `parse_scenes`.
