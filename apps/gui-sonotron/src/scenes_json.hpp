...
// storage shape). Fewer than kMaxSceneCount entries in ANY array leaves the
// remaining indices at whatever `model` already had (its own constructor
// default: the bare column number for names, GridModel::kDefaultSectionType
// for sections, GridModel::kDefaultSceneBars for bars); more than
// kMaxSceneCount entries are ignored past that bound, forward-compatible
// with a file written by a future binary with a larger grid. The
// "sections"/"bars" keys are OPTIONAL: a scenes.json written by an older
// binary (missing either key, or both) parses cleanly and every scene keeps
// the constructor default for the missing field(s) -- old files are not a
// parse failure. Returns true on success; on failure returns false and
// fills `error` with a 1-based line/column message, leaving `model`
// partially updated (same "parse failure is a whole-file failure"
// discipline as layout_json.hpp's parse_layout -- the caller is expected to
// discard `model`'s scene state on a `false` return, not trust a partial
// parse).
...
// AND per-scene bar lengths to canonical JSON text (deterministic key order
// -- "scenes" then "sections" then "bars" -- fixed 2-space indentation, no
// trailing newline -- callers add one), the exact on-disk format read back
// by `parse_scenes`.
