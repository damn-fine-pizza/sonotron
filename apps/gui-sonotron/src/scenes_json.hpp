...
// array. Fewer than kMaxSceneCount entries leaves the remaining indices at
// whatever `model` already had (its own constructor default, the bare
// column number); more than kMaxSceneCount entries are ignored past that
// bound, forward-compatible with a file written by a future binary with a
// larger grid. Returns true on success; on failure returns false and fills
// `error` with a 1-based line/column message, leaving `model` partially
// updated (same "parse failure is a whole-file failure" discipline as
// layout_json.hpp's parse_layout -- the caller is expected to discard
// `model`'s scene names on a `false` return, not trust a partial parse).
bool parse_scenes(const std::string& text, GridModel& model, std::string& error);

// Serializes every one of `model`'s kMaxSceneCount scene names to canonical
// JSON text (deterministic key order, fixed 2-space indentation, no
